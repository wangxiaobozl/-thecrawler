...
def pqgc(ip_temp,ym):
    ...
    url = "http://app1.sfda.gov.cn/datasearch/face3/base.jsp?tableId=25&tableName=TABLE25&title=%B9%FA%B2%FA%D2%A9%C6%B7&bcId=124356560303886909015737447882"  # 这里打开我的博客网站
        ...
        for yp_item in yplist:
            ...
            if i == 0:
                yp_id = yp_item.get_attribute("href").replace(
                    "javascript:commitForECMA(callbackC,\"content.jsp?tableId=25&tableName=TABLE25&tableView=%E5%9B%BD%E4%BA%A7%E8%8D%AF%E5%93%81&Id=",
                    '').replace("\",null)", "")
            else:
                yp_id = yp_item.get_attribute("href").replace(
                    "javascript:commitForECMA(callbackC,'content.jsp?tableId=25&tableName=TABLE25&tableView=%E5%9B%BD%E4%BA%A7%E8%8D%AF%E5%93%81&Id=",
            ...
            #sql = "insert into yplist (nameinfo,number) VALUE ('" + yp_name + "','" + yp_id + "')"
            sql = "update yplist set nameinfo='" + yp_name + "',number='" + yp_id + "' where id="+yp_name[:int(yp_name.index('.'))]
            cursor.execute(sql)
            connection.commit()
